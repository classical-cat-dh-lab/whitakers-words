...
self.addEventListener('message',async({data})=>{
  try{
    ...
    const result=data.mode==='english'?analyzer.lookupEnglish(data.input):analyzer.analyze(data.input);
    self.postMessage({type:'result',id:data.id,result:data.mode==='corrected'?applyCorrectedLayer(result):result,milliseconds:performance.now()-start});
