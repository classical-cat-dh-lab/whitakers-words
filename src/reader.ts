...
export function presentAnalysis(result: Analysis | EnglishResult): ReaderResult {
    if ('hits' in result) {
        return { version: READER_VERSION, language: 'english', tokens: [{ surface: result.input, status: result.status ?? (result.hits.length ? 'analyzed' : 'unknown'), notes: result.trimmed ? ['Additional entries were omitted by the legacy display profile.'] : [], items: result.hits.map(({ entry }, index) => ({ kind: 'entry', title: dictionaryForms(entry.citation), labels: labels(entry.part.pos, entry), forms: [], meaning: entry.meaning.replace(/\r/g, '').trim(), details: entryDetails(entry), sourceIndices: [index] })) }], notes: result.diagnostics ?? [] };
    }
